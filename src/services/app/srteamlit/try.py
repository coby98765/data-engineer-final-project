...
# טוענים את GeoJSON של כל הערים
with open("municipalities.geojson", "r", encoding="utf-8") as f:
    geo_data = json.load(f)

# יוצרים רשימת ערים מתוך ה-GeoJSON
all_cities = [feature['properties']['MUN_HEB'] for feature in geo_data['features']]

def show_city_map(selected_cities):
    # מפה מרכזית בישראל
    m = folium.Map(location=[31.5, 34.8], zoom_start=8)

    # פונקציה לצביעת הערים
    def style_function(feature):
        city_name = feature['properties']['MUN_HEB']
...
# Streamlit: מאפשר למשתמש לבחור כמה ערים
selected_cities = st.multiselect("בחר ערים:", all_cities)
map_to_show = show_city_map(selected_cities)
st_data = st_folium(map_to_show, width=800, height=600)